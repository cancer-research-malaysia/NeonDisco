...
LABEL version="1.0"
LABEL description="minimal container image for Reditools2 for CRMY"
...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends build-essential libssl-dev libffi-dev libxml2-dev libxslt1-dev zlib1g-dev libbz2-dev zlib1g-dev libncurses5-dev libncursesw5-dev liblzma-dev curl wget tar pigz gzip zip unzip gcc g++ bzip2 procps coreutils gawk grep sed nano less git tabix openmpi-common libopenmpi-dev \
...
COPY --chown=$MAMBA_USER:$MAMBA_USER reditools2/context/base_env.yaml /tmp/base_env.yaml
...
RUN pip install fisher==0.1.4

# install REDItools1
RUN git clone https://github.com/tflati/reditools2.0.git && cd reditools2.0 
