"""
Holds classes for containing information for Gene and Fusion exon and protein information. PATCHED version v1.4.4 sufyazi
"""
...
class FusionTranscript:
    """
    Generates the information needed for the gene fusion transcripts
    """
    ...
    def _annotate(self):
        """
        Annotate the gene fusion's protein using the protein annotation
        from its two genes
        """
    ...
    def _fetch_protein(self):
        """
        Predict the potential protein amino acid sequence
        """
    ...
    def _fetch_5utr_transcript2(self):
        """
        Fetch the 5'UTR sequence from transcript2 when the junction
        falls in the 5'UTR region (before the first CDS).
        This ensures correct frame prediction for the fusion.
        
        This fix addresses: https://github.com/murphycj/AGFusion/issues/4
        """
        exons = self.transcript2.exon_intervals
        first_cds = None
        
        if self.transcript2.contains_start_codon and self.transcript2.contains_stop_codon:
            # Get first CDS position range
            first_cds = self.transcript2.coding_sequence_position_ranges[0]
        else:
            return ''
        
        running_sum = 0
        utr_seq = ''
        rev_strand = (self.transcript2.strand == "-")
        
        for i, exon in enumerate(exons):
            exon_length = exon[1] - exon[0] + 1
            
            # Check if exon is not in the fusion
            if (not rev_strand and self.gene3prime.junction > exon[1]) or \
               (rev_strand and self.gene3prime.junction < exon[0]):
                running_sum += exon_length
                continue
            else:
                # Check if exon overlaps with first CDS
                if (first_cds[0] <= exon[1] and first_cds[1] >= exon[0]):
                    # Exon contains start of CDS, get the UTR portion
                    
                    if not rev_strand:
                        if self.gene3prime.junction >= exon[0] and self.gene3prime.junction <= exon[1]:
                            # Junction is within this exon
                            utr_seq += self.transcript2.sequence[
                                running_sum + (self.gene3prime.junction - exon[0]):
                                running_sum + (first_cds[0] - exon[0])
                            ]
                        else:
                            # Junction is not in this exon
                            utr_seq += self.transcript2.sequence[
                                running_sum:
                                running_sum + (first_cds[0] - exon[0])
                            ]
                        ...
                        if self.gene3prime.junction >= exon[0] and self.gene3prime.junction <= exon[1]:
                            # Junction is within this exon
                            utr_seq += self.transcript2.sequence[
                                running_sum + (exon[1] - self.gene3prime.junction):
                                running_sum + (exon[1] - first_cds[1])
                            ]
                        else:
                            # Junction is not in this exon
                            utr_seq += self.transcript2.sequence[
                    ...
                    # Exon does not contain CDS but does contain the fusion junction
                    # Get the UTR portion
                    if not rev_strand:
                        if self.gene3prime.junction >= exon[0] and self.gene3prime.junction <= exon[1]:
                            # Junction is within this exon
    ...
    def _fetch_transcript_cds(self):
        ...
        # Check if 3' gene junction is in the 5'UTR
        # If so, include the 5'UTR sequence in fusion CDS (fixes issue #4)
        utr_seq = ''
        if self.transcript2.contains_start_codon and self.transcript2.contains_stop_codon:
            first_cds = self.transcript2.coding_sequence_position_ranges[0]

            if self.transcript2.strand == "+":
                if self.gene3prime.junction < first_cds[0]:
                    utr_seq = self._fetch_5utr_transcript2()
            else:
                if self.gene3prime.junction > first_cds[1]:
                    utr_seq = self._fetch_5utr_transcript2()

        # Get the CDS sequence with UTR prepended if applicable
        self.cds_3prime = utr_seq + self.transcript2.coding_sequence[self.transcript_cds_junction_3prime : :]
