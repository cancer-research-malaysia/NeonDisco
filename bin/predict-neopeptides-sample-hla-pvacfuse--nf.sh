...
if pvacfuse run ${VALIDATED_AGF_DIR} ${SAMPLE_NAME} ${SSHLA} BigMHC_EL BigMHC_IM DeepImmuno MHCflurry MHCflurryEL NetMHCpanEL NetMHCcons SMMPMBEC "${OUTPUT_DIR}" --iedb-install-directory "/opt/iedb" --allele-specific-binding-thresholds --run-reference-proteome-similarity --peptide-fasta ${METADATA_DIR}/Homo_sapiens.GRCh38.pep.all.fa.gz --netmhc-stab -t ${NUM_CORES} -a sample_name 2>&1 | tee -a "$REPORT_FILE"; then
