...
arcasHLA extract ${BAM} -t ${CORES} -v 

if [ -f *.extracted.1.fq.gz && -f *.extracted.2.fq.gz ]; then
    echo "arcasHLA extract run successfully for sample ${SAMPLE_ID}."
    arcasHLA genotype *.1.fq.gz *.2.fq.gz -t ${CORES} -v
    if [ $? -eq 0 ]; then
    ...
    echo "arcasHLA extract failed for sample ${SAMPLE_ID}."
    exit 1
fi
